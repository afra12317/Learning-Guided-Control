...
class MPPI:
    ...
    def __init__(self, config, env, jrng, temperature=0.01, damping=0.001, track=None):
        self.config = config
    ...
    def update(self, env_state, reference_traj, a_init=None):
        ...
        if (
            ...
            and self.config.state_predictor in self.config.cartesian_models
        ):
    ...
    def iteration_step(self, a_opt, a_cov, rng_da, env_state, reference_traj):
        ...
        low = -jnp.ones((self.n_samples, self.n_steps, self.a_shape)) * self.a_std
        high = jnp.ones((self.n_samples, self.n_steps, self.a_shape)) * self.a_std
        ...
        R = jax.vmap(self.returns)(
            reward
        )  # [n_samples, n_steps], pylint: disable=invalid-name
        ...
        if self.adaptive_covariance:
            ...
            a_cov = (
                a_cov + jnp.eye(self.a_shape) * 0.00001
    ...
    def weights(self, R):  # pylint: disable=invalid-name
        ...
        R_stdzd = (R - jnp.max(R)) / (
            (jnp.max(R) - jnp.min(R)) + self.damping

...
class MPPI:
    ...
    def __init__(self, config, env, jrng, temperature=0.005, damping=0.001, track=None):
        self.config = config
        ...
        self.accum_matrix = jax.device_put(
            jnp.triu(jnp.ones((self.n_steps, self.n_steps)))
        )
        self.track = track
    ...
    def init_state(self, env, a_shape):
        ...
        self.env = env
        self.a_opt = 0.0 * jax.random.uniform(
            self.jrng.new_key(), shape=(self.n_steps, dim_a)
        )
    ...
    def mppi_loop(self, a_opt, a_cov, env_state, reference_traj, obs_array, key):
        def body_fn(carry, _):
            ...
            a_opt, a_cov, states, traj = self.iteration_step(
                a_opt, a_cov, subkey, env_state, reference_traj, obs_array
            )
            return (a_opt, a_cov, key), (states, traj)

        (a_opt, a_cov, _), (all_states, all_trajs) = lax.scan(
            body_fn, (a_opt, a_cov, key), xs=None, length=self.n_iterations
        )
    ...
    def check_collision_batch(states, obstacle_points, collision_radius=0.2):
    ...
    @partial(jax.jit, static_argnums=(0,))
    def iteration_step(
        self, a_opt, a_cov, rng_da, env_state, reference_traj, obs_array
    ):
        rng_da, rng_da_split1, rng_da_split2 = jax.random.split(rng_da, 3)
        da, actions, states = self._jit_rollout_block(
            a_opt, rng_da, rng_da_split1, env_state
        )

        # states: [n_samples, T, state_dim]
        # obs_array: [max_obs, 2] (padded with invalid values, e.g. self.config.invalid_pos)
        collision_flags = jax.vmap(self.check_collision_batch, in_axes=(0, None))(
            states, obs_array
        )
    ...
    def rollout_batch(self, actions_batch, env_states, rng_key):
        return jax.vmap(self.rollout, in_axes=(0, 0, None))(
            actions_batch, env_states, rng_key
        )
